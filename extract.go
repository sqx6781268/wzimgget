...
func cmdExtract(args []string) {
	...
	// 这里手动分离 data 位置参数与选项，使 `extract <data> -out X -v` 任意顺序均可。
	...
	err = filepath.Walk(inAbs, func(path string, info os.FileInfo, err error) error {
		...
		st.total++
		res, err := extractOne(path, inAbs, outAbs)
		if err != nil {
			st.failed++
...
func extractOne(path, inAbs, outAbs string) (string, error) {
	...
	if decodeErr != nil {
		return "", fmt.Errorf("解码图标失败: %w", decodeErr)
	}
	return "", nil
}
